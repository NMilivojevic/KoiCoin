...
export const initializeDatabase = async () => {
  try {
    ...
    try {
      ...
      const hasCurrencyColumn = tableInfo.rows.some((row: any) => row.name === 'currency');
      
      if (!hasCurrencyColumn) {
        await query(`ALTER TABLE users ADD COLUMN currency TEXT DEFAULT 'RSD'`);
    ...
    try {
      const accountsTableInfo = await query(`PRAGMA table_info(accounts)`);
      const hasCurrencyColumn = accountsTableInfo.rows.some((row: any) => row.name === 'currency');
      
      if (!hasCurrencyColumn) {
        await query(`ALTER TABLE accounts ADD COLUMN currency TEXT DEFAULT 'RSD'`);
    ...
    try {
      const transactionsTableInfo = await query(`PRAGMA table_info(transactions)`);
      const hasCurrencyColumn = transactionsTableInfo.rows.some((row: any) => row.name === 'currency');
      
      if (!hasCurrencyColumn) {
        await query(`ALTER TABLE transactions ADD COLUMN currency TEXT DEFAULT 'RSD'`);
    ...
    try {
      ...
      await query(`CREATE INDEX IF NOT EXISTS idx_transactions_account_id ON transactions(account_id)`);
      ...
      await query(`CREATE INDEX IF NOT EXISTS idx_categories_type ON categories(type)`);
